...
impl SecretShare {
    fn encrypt(
        ...
        EncryptedSecretShare(self.0 + transcript.challenge_scalar(b"ecdh"))
    }
}
...
pub struct EncryptedSecretShare(Scalar);
...
pub mod round2 {
    use super::{
        round1, Certificate, EncryptedSecretShare, Identifier, Parameters, Participants,
        ...
        for (i, (identifier, private_message)) in round2_private_messages.iter().enumerate() {
            let secret_share = private_message.encrypted_secret_share().decrypt(
                ...
                &round1_public_messages[i]
                    ...
                    .decompress()
                    .unwrap(),
                participants.own_identifier.0.as_bytes(),
            );
