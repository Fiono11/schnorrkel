...
impl VRFProofBatchable {
    ...
    pub fn from_bytes(bytes: &[u8]) -> SignatureResult<VRFProofBatchable> {
        ...
        let s = crate::scalar_from_canonical_bytes(s).ok_or(SignatureError::ScalarFormatError)?;
        Ok(VRFProofBatchable {
            R: CompressedRistretto(R),
            Hr: CompressedRistretto(Hr),
            s,
        })
    }
...
pub fn dleq_verify_batch(
    ...
    let z_c: Vec<Scalar> = zz
        ...
        .map(|(i, z)| {
            z * proofs[i]
                .shorten_dleq(t0.clone(), &public_keys[i], &ps[i], kusama)
                .c
        })
        .collect();
...
mod tests {
    ...
    fn vrf_malleable() {
        ...
        assert_eq!(
            ...
            proofs21
                .1
                .shorten_dleq(t0.clone(), &keypair2.public, &io21, KUSAMA_VRF),
        ...
        assert_eq!(
            ...
            proofs12
                .1
                .shorten_dleq(t0.clone(), &keypair1.public, &io12, KUSAMA_VRF),
            "Oops `shorten_dleq` failed"
        );
        assert!(keypair1
            .public
