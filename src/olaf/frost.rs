...
use super::{simplpedpop::TotalSecretShare, GroupPublicKey, GroupPublicKeyShare};
...
pub(crate) struct BindingFactor(Scalar);
...
pub mod round1 {
    ...
    use crate::{context::SigningTranscript, olaf::Identifier};
    use alloc::vec::Vec;
...
pub mod round2 {
    use super::{round1, BindingFactor, Challenge, SigningShare, VerifyingKey, VerifyingShare};
    use crate::olaf::GENERATOR;
    use crate::{
        context::SigningTranscript,
        olaf::{
            error::FROSTError, polynomial::PolynomialCommitment,
            simplpedpop::SecretPolynomialCommitment, Identifier,
        },
        Signature,
    };
    use alloc::{
    ...
    impl PublicKeyPackage {
        ...
        pub fn from_commitment(
            ...
            let verifying_keys: BTreeMap<_, _> = identifiers
                .iter()
                .map(|id| (*id, commitment.evaluate(&id.0)))
                .collect();
    ...
    fn challenge(R: &RistrettoPoint, verifying_key: &VerifyingKey, msg: &[u8]) -> Challenge {
