...
/// A wrapper around CompressedRistretto that implements PartialOrd and Ord traits.
#[derive(Debug, Copy, Clone, Default, PartialEq, Eq)]
pub struct Element(pub(crate) CompressedRistretto);

impl PartialOrd<Element> for Element {
    fn partial_cmp(&self, other: &Element) -> Option<::core::cmp::Ordering> {
        Some(self.cmp(other))
    }
}
